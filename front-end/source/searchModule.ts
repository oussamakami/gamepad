...
class SearchLoader {
    ...
    private createItem(userid, username, onlineStatus = false): HTMLElement {
        ...
        btnContainer.appendChild(this.btnGenerator.generateAnchorButton(`/profile?id=${userid}`, "Profile"));
        btnContainer.appendChild(this.btnGenerator.generateActionButton("block", userid));
    ...
    private updatePageBody(): void {
        ...
        while (start < end) {
            ...
            fragment.appendChild(this.createItem(userid, username));
            start++;
        }
